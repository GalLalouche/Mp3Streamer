...
class MbHtmlLinkExtractorTest extends FreeSpec with AuxSpecs {
  ...
  "parse artist links" in {
    val $ = new ArtistLinkExtractor() {
      override private[mb] def getHtml(artistId: String): Future[Document] = getDocument("artist")
    }
    val expected = Set(
  ...
  "parse album links" in {
    val $ = new AlbumLinkExtractor() {
      override private[mb] def getHtml(artistId: String): Future[Document] = getDocument("album")
    }
    val expected = Set(
