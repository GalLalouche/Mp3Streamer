...
import controllers.AssetHttpRoutes.asset
import http4s.Http4sUtils
import http4s.Http4sUtils.decodePath
import org.http4s.{HttpRoutes, Request, Response}
import org.http4s.dsl.io._

// Since 2.6 ruined their own assets controller :\
class AssetHttpRoutes @Inject() {
  val routes: HttpRoutes[IO] = HttpRoutes.of[IO] {
    case req @ GET -> "assets" /: path =>
      asset(req, decodePath(path))
    case req @ GET -> "js" /: path =>
      asset(req, "javascripts\\" + decodePath(path))
