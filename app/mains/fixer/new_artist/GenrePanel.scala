...
private[fixer] class GenrePanel private (
      ...
      .map(
        add(
          genreBox(
            ...
            sideMultiplayer = bigIconMultiplayer,
            fontSize = Some(35),
          ),
          _,
        ),
  ...
  private def updateSelection(s: String)(d: Direction): Unit = {
    def select(newIndex: Int): Unit = {
      ...
      boxes.zipWithIndex.foreach { case (b, i) =>
  ...
  private def dynamicConstraint(
      relativeGenreIndex: Int,
      width: Int,
      height: Int,
  ): UpdatingColumns[Constraints] =
    State.get.map(usedUpColumns =>
      constraints(
  ...
  private def constraints(gridX: Int, gridY: Int, width: Int, height: Int): Constraints =
    new Constraints(
      gridx = gridX,
      gridy = gridY,
      gridwidth = width,
