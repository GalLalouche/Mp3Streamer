...
trait InternetTalker extends ExecutionContext {
  ...
  def asBrowser(url: Url): WSRequest = createWsClient().url(url.address).withHeaders("user-agent" ->
      "user-agent:Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.86 Safari/537.36")
  def downloadDocument(url: Url): Future[Document] = asBrowser(url).document
  def get(url: Url): Future[WSResponse] = createWsClient().url(url.address).get()
  def createWsClient(): WSClient
}
