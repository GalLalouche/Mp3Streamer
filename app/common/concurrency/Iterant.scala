...
/** A non-sealed alternative to RichStreamT which is possible to extend using different implementations. */
abstract class Iterant[F[_] : Monad, A] {
  ...
  def batchStep(n: Int, onNext: A => Unit = _ => (), onComplete: () => Unit = () => ()): F[(Seq[A], Iterant[F, A])] = {
    lazy val default = (Seq.empty[A] -> this).point
    if (n == 0) default else step.run.flatMap {
      case None =>
        onComplete()
        default
      case Some((head, tail)) =>
        onNext(head)
        tail.batchStep(n - 1, onNext, onComplete).map(TuplePLenses.tuple2First.modify(head :: _.toList))
    }
  }
  def toStream: StreamT[F, A] = StreamT(step.fold(e => StreamT.Yield(e._1, e._2.toStream), StreamT.Done))
...
object Iterant {
  ...
  def range[F[_] : Monad](start: Int, until: Int, by: Int = 1): Iterant[F, Int] = new Iterant[F, Int] {
    override def step =
      if (start >= until)
        OptionT.none
      else
  ...
  def fromStream[F[_] : Monad, A]($: StreamT[F, A]): Iterant[F, A] = new Iterant[F, A] {
    override def step = OptionT[F, (A, StreamT[F, A])]($.uncons).map {
      case (next, s) => (next, fromStream(s))
    }
    override def toStream = $
  }

  def fromProducer[A]($: AsyncProducer[A])(
      implicit ec: ExecutionContext): FutureIterant[A] = fromStream(RichStreamT.fillM(OptionT($.!())))

  def prefetching[F[_] : Monad, A]($: Iterant[F, A], n: Int): Iterant[F, A] = PrefetchingIterant[F, A]($, n)
}
