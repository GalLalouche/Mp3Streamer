...
import common.rich.func.{MoreSeqInstances, ToMoreMonadPlusOps}
...
class CompositeDateFormat private(formatters: Seq[DateTimeFormatter])
    extends MoreSeqInstances with ToMoreMonadPlusOps {
  // TODO replace with NonEmptyList
  require(formatters.nonEmpty)

  def parse(source: String): Option[LocalDateTime] =
    formatters.tryMap(LocalDateTime.parse(source, _)).headOption
...
object CompositeDateFormat {
  def apply(patterns: String*) = new CompositeDateFormat(
    patterns.map(DateTimeFormatter.ofPattern).map(_.withZone(ZoneId.systemDefault)))
}
