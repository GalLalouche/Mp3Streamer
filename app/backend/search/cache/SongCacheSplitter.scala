...
private class SongCacheSplitter @Inject()(
  ...
  def apply(cs: SongCache)(
    ...
    val albums = log("saving albums")(songs
        .groupBy(_.file.asInstanceOf[IOFile].file.getParent)
        .map {case (parent, songs) =>
          val firstSong = songs.head
          Album(IODirectory(parent), firstSong.albumName, firstSong.artistName, firstSong.year, songs.toVector)
        }.toSet <| jsonableSaver.saveArray
    )
    log("saving artists") {
      val artists = albums.groupBy(_.artistName).map {
        case (artistName, albums) => Artist(artistName, albums)
      }
