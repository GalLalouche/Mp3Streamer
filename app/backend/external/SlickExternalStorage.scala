...
class SlickExternalStorage[K <: Reconcilable](implicit c: Configuration,
  ...
  private def toString(els: Traversable[ExternalLink[K]]): String = els map Serializable.encode mkString ";;"
  private def fromString(s: String): Traversable[ExternalLink[K]] = s split ";;" map Serializable.decode
  private def store(k: K, els: Traversable[ExternalLink[K]], t: Option[Long]): Future[Unit] =
    db.run(rows.forceInsert((k.normalize, els |> toString, t))).map(e => Unit)
  override def load(k: K): Future[Option[(Traversable[ExternalLink[K]], Option[DateTime])]] =
    db.run(rows
  ...
  override protected def internalForceStore(k: K, v: (Traversable[ExternalLink[K]], Option[DateTime])): Future[Unit] =
