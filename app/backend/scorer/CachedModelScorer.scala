...
*/
class CachedModelScorer @Inject()(
  ...
  def apply(a: Album): Option[ModelScore] = albumScores.get(a.artist, a.title.toLowerCase)
  def apply(s: Song): Option[ModelScore] = aux(s).toModelScore
