...
import scalaz.Scalaz.{eitherInstance, optionInstance, ToMonadPlusOpsUnapply, ToTuple2Ops}
...
class ScoreParser @Inject()(
  ...
  private def store(scores: Seq[(Either[Artist, Album], ModelScore)]): Future[Unit] = {
    ...
    artistScoreStorage.overwriteMultipleVoid(artists)
    albumScoreStorage.overwriteMultipleVoid(albums)
  }
