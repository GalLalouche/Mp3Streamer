...
import scalaz.Scalaz.{ToApplyOps, ToBindOps, ToFunctorOpsUnapply}
...
private class CachedNewAlbumStorageImpl @Inject()(
  ...
  override def all = newAlbumStorage.all.map {
    t => t.&|->(ArtistNewAlbums.albums).modify(filterExistingAlbums(t.artist, _))
  ...
  override def isIgnored(artist: Artist) = lastFetchTime.freshness(artist).run.map(
