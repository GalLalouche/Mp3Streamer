...
(function($, undefined) {
  ...
  $.fn.jPlayer = function(options) {
    ...
    if (isMethodCall) {
      this.each(function() {
        ...
        const methodValue = instance && $.isFunction(instance[options]) ?
  ...
  $.jPlayer.convertTime = function(s) {
    ...
    const min = myTime.getUTCMinutes()
    const sec = myTime.getUTCSeconds()
    const strHour = ($.jPlayer.timeFormat.padHour && hour < 10) ? "0" + hour : hour
    const strMin = ($.jPlayer.timeFormat.padMin && min < 10) ? "0" + min : min
    const strSec = ($.jPlayer.timeFormat.padSec && sec < 10) ? "0" + sec : sec
  ...
  $.jPlayer.uaPlatform = function(userAgent) {
    ...
    // Useragent RegExp
    const rplatform = /(ipad|iphone|ipod|android|blackberry|playbook|windows ce|webos)/
    const rtablet = /(ipad|playbook)/
    const randroid = /(android)/
    const rmobile = /(mobile)/

    const platform = rplatform.exec(ua) || []
    const tablet = rtablet.exec(ua) ||
  ...
  const browserMatch = $.jPlayer.uaBrowser(navigator.userAgent)
  if (browserMatch.browser) {
    $.jPlayer.browser[browserMatch.browser] = true
    $.jPlayer.browser.version = browserMatch.version
  }
  const platformMatch = $.jPlayer.uaPlatform(navigator.userAgent)
  ...
  $.jPlayer.prototype = {
    ...
    options: { // Instanced in $.jPlayer() constructor
      ...
      autobuffer: false, // setting this to false seems to help mobile buffering
    ...
    _init: function() {
      ...
      $.each(this.options.supplied.toLowerCase().split(","), function(index1, value1) {
        const format = value1.replace(/^\s+|\s+$/g, "") //trim
      ...
      $.each(this.options.solution.toLowerCase().split(","), function(index1, value1) {
        const solution = value1.replace(/^\s+|\s+$/g, "") //trim
      ...
      if (this.flash.used) {
        let htmlObj
        const flashVars = 'jQuery=' + encodeURI(this.options.noConflict) + '&id=' + encodeURI(this.internal.self.id) + '&vol=' + this.options.volume + '&muted=' + this.options.muted

        // Code influenced by SWFObject 2.2: http://code.google.com/p/swfobject/
        // Non IE browsers have an initial Flash size of 1 by 1 otherwise the wmode affected the Flash ready event.

        if ($.jPlayer.browser.msie && Number($.jPlayer.browser.version) <= 8) {
          const objStr = '<object id="' + this.internal.flash.id + '" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="0" height="0"></object>'
          ...
          const createParam = function(el, n, v) {
    ...
    _getHtmlStatus: function(media, override) {
      ...
      let currentPercentageAbsolute = (this.status.duration > 0) ? 100 * currentTime / this.status.duration : 0
      if ((typeof media.seekable === "object") && (media.seekable.length > 0)) {
        seekPercent = (this.status.duration > 0) ? 100 * media.seekable.end(media.seekable.length - 1) / this.status.duration : 100
        currentPercentageRelative = (this.status.duration > 0) ? 100 * media.currentTime / media.seekable.end(media.seekable.length - 1) : 0; // Duration conditional for iOS duration bug. ie., seekable.end is a NaN in that case.
      } else {
        seekPercent = 100
        currentPercentageRelative = currentPercentageAbsolute
      }

      if (override) {
        currentTime = 0
        currentPercentageRelative = 0
        currentPercentageAbsolute = 0
      }

      this.status.seekPercent = seekPercent
      this.status.currentPercentRelative = currentPercentageRelative
      this.status.currentPercentAbsolute = currentPercentageAbsolute
    ...
    volumeBar: function(e) { // Handles clicks on the volumeBar
      if (this.css.jq.volumeBar.length) {
        const offset = this.css.jq.volumeBar.offset()
        const x = e.pageX - offset.left
        const w = this.css.jq.volumeBar.width()
        const y = this.css.jq.volumeBar.height() - e.pageY + offset.top
        const h = this.css.jq.volumeBar.height()
    ...
    seekBar: function(e) { // Handles clicks on the seekBar
      if (this.css.jq.seekBar) {
        const offset = this.css.jq.seekBar.offset()
        const x = e.pageX - offset.left
        const w = this.css.jq.seekBar.width()
        const p = 100 * x / w
        this.playHead(p)
      }
    },
    ...
    option: function(key, value) {
      ...
      if (typeof key === "string") {
        ...
        if (value === undefined) {
          ...
          let opt = $.extend(true, {}, this.options)
    ...
    _updateAutohide: function() {
      const self = this
      const event = "mousemove.jPlayer"
      const namespace = ".jPlayerAutohide"
      const eventType = event + namespace
    ...
    _destroyHtmlBridge: function() {
      ...
      const emulated = $.jPlayer.emulateMethods + " " + $.jPlayer.emulateStatus + " " + $.jPlayer.emulateOptions
