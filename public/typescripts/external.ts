...
class Helper {
  ...
  constructor() {
    ...
    button("Refresh").appendTo(this.externalArtist).click(this.refresh("artist"))
    button("Refresh").appendTo(this.externalAlbum).click(this.refresh("album"))
    // Update recon on pressing Enter
    validateBoxAndButton($(".external-recon-id"), updateReconButton, s => RECON_REGEX.test(s), this.updateRecon)
